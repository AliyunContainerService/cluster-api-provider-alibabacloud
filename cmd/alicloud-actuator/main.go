...
func init() {
	...
	rootCmd.PersistentFlags().StringP("alicloud-credentials", "a", "", "Secret manifest with alicloud credentials")
...
var rootCmd = &cobra.Command{
	Use:   "alicloud-actuator-test",
	Short: "Test for Cluster API AliCloud actuator",
}
...
func createCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			machine, alicloudCredentials, userData, err := readClusterResources(
				&manifestParams{
					ClusterID: cmd.Flag("environment-id").Value.String(),
				},
				cmd.Flag("machine").Value.String(),
				cmd.Flag("alicloud-credentials").Value.String(),
